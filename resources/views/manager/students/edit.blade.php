        ...
        <div class="col-md-12">
            <div class="card mb-4">
                <div class="card-header d-flex align-items-center justify-content-between">
                    ...
                    <div>
                        <a href="{{ route('manager.students.show', $student) }}" class="btn btn-outline-info me-2">
                            <i class="bx bx-show me-1"></i>View Student
                        </a>
                ...
                <div class="card-body">
                    <form action="{{ route('manager.students.update', $student) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="row mb-4">
                                <div class="col-12">
                                    <h6 class="text-primary mb-3">
                                        <i class="bx bx-image me-1"></i>Current Photo
                                    </h6>
                                    <div class="d-flex align-items-center">
                                        <img src="{{ asset('storage/' . $student->image) }}" 
                                             ...
                                             width="80" height="80">
                                        <div>
                                            <p class="mb-1">Current student photo</p>
                                            <small class="text-muted">Upload a new photo below to replace this image</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('std_number') is-invalid @enderror" 
                                       id="std_number" name="std_number" value="{{ old('std_number', $student->std_number) }}" required>
                                @error('std_number')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                <label for="image" class="form-label">Update Student Photo</label>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('genotype') is-invalid @enderror" id="genotype" name="genotype">
                                    ...
                                    <option value="AA" {{ old('genotype', $student->genotype) == 'AA' ? 'selected' : '' }}>AA</option>
                                    <option value="AS" {{ old('genotype', $student->genotype) == 'AS' ? 'selected' : '' }}>AS</option>
                                    <option value="SS" {{ old('genotype', $student->genotype) == 'SS' ? 'selected' : '' }}>SS</option>
                                    <option value="AC" {{ old('genotype', $student->genotype) == 'AC' ? 'selected' : '' }}>AC</option>
                                    <option value="SC" {{ old('genotype', $student->genotype) == 'SC' ? 'selected' : '' }}>SC</option>
                                </select>
                                @error('genotype')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('bgroup') is-invalid @enderror" id="bgroup" name="bgroup">
                                    ...
                                    <option value="A+" {{ old('bgroup', $student->bgroup) == 'A+' ? 'selected' : '' }}>A+</option>
                                    <option value="A-" {{ old('bgroup', $student->bgroup) == 'A-' ? 'selected' : '' }}>A-</option>
                                    <option value="B+" {{ old('bgroup', $student->bgroup) == 'B+' ? 'selected' : '' }}>B+</option>
                                    <option value="B-" {{ old('bgroup', $student->bgroup) == 'B-' ? 'selected' : '' }}>B-</option>
                                    <option value="AB+" {{ old('bgroup', $student->bgroup) == 'AB+' ? 'selected' : '' }}>AB+</option>
                                    <option value="AB-" {{ old('bgroup', $student->bgroup) == 'AB-' ? 'selected' : '' }}>AB-</option>
                                    <option value="O+" {{ old('bgroup', $student->bgroup) == 'O+' ? 'selected' : '' }}>O+</option>
                                    <option value="O-" {{ old('bgroup', $student->bgroup) == 'O-' ? 'selected' : '' }}>O-</option>
                                </select>
                                @error('bgroup')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-select @error('current_session') is-invalid @enderror" id="current_session" name="current_session" required>
                                    ...
                                    @foreach($sessions as $session)
                                        <option value="{{ $session->id }}" {{ old('current_session', $student->current_session) == $session->id ? 'selected' : '' }}>
                                            {{ $session->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-12">
                                ...
                                <div class="d-flex justify-content-between">
                                    ...
                                    <button type="submit" class="btn btn-primary">
                                        <i class="bx bx-check me-1"></i>Update Student
                                    </button>
                                </div>
                            </div>
                        </div>
